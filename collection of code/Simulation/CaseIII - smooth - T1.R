...
# just substitute the functions of "ipw_U_stoc" and "ipw_T_stoc" in the following code
# Specifically, just substitute  "g"
# IPW
ipw_U_stoc <- function(beta, x, y, a, prob, tao, delta, G_c_tao) {
  ...
  c0 <- 4^(1/3)
  betaX <- x %*% beta
  h <- c0*length(a)^(- 1/3)*sd(betaX)
  g <- pnorm(betaX/h, mean = 0, sd = 1)
...
result = break_result(result_ipwT_d1_40)
cat(
...
MaxT_ipw_p1(q_cons=0.01, n=1000, gamma_p, gamma_t, lambda_c, sim_num = 1) # test
registerDoRNG(2023);a0 = proc.time()[3]
...
result = break_result(result_T_ipw_p1_42)
cat(
...
result_U_ipw_p1_20 = foreach(i=1:10, .combine='rbind', .multicombine = TRUE) %dopar% {
  MaxU_ipw_p2(q_cons=2.0, n=1000, gamma_p, gamma_t, lambda_c, sim_num = 50)};print(proc.time()[3] - a0)
registerDoRNG(2023);a0 = proc.time()[3]
result_U_ipw_p1_23 = foreach(i=1:10, .combine='rbind', .multicombine = TRUE) %dopar% {
  MaxU_ipw_p2(q_cons=2.3, n=1000, gamma_p, gamma_t, lambda_c, sim_num = 50)};print(proc.time()[3] - a0)
registerDoRNG(2023);a0 = proc.time()[3]
...
result = break_result(result_U_ipw_p1_26)
cat(
  "beta", apply(result[[1]], 2, mean),"\n",
  "beta_sd", apply(result[[1]], 2, sd),"\n",
  "V", apply(result[[2]], 2, mean),"\n",
